/*  eBookDys.js — a set of helpers for dyslexic readers
        ...
        It’s a 0.001 version by the way
		...
		- fix Readium (for some reason it works in ADE 4 but not Readium ext…) 
 																																		*/
...
r(function() {
	...
	if ('touchend' in window) {  // I can hear you scream, Baldur.
		isNotTouch = false; 
	}
	
	// For injecting eBookDys.css automatically
	var loadCSS = true; // Change to false if you don’t want auto-injection
	var eBookDysCSS = '../Styles/eBookDys.css'; // path to the css
  ...
  // For injecting menu styles
	...
	for (var i = 0; i < colors.length; i++) {
	  ...
	  // Creating a button
		...
		// Adding eventListeners for button — Yeah I know, event delegation etc.
  ...
  if ('speechSynthesis' in window) {
		window.utterances = []; // We need this because of crappy implementations
		var ttsLabel = document.createElement('label');
		var isPaused = true; // We need this because of crappy implementations
	...
	// Reading rule
    // By now Baldur should be so pissed by this script that he’s thinking about refactoring it
